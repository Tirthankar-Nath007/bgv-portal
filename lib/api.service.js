...
async function apiRequest(endpoint, options = {}) {
  ...
  try {
    ...
    // If unauthorized — remove token and redirect to appropriate login page
    if (response.status === 401) {
      console.error('401 Unauthorized - token may be invalid or expired');
      removeToken();
      if (typeof window !== 'undefined') {
        // Check which session was being used and redirect accordingly
        const currentPath = window.location.pathname;
        if (currentPath.startsWith('/admin')) {
...
const authAPI = {
  ...
  register: async (companyName, email, password, isBgvAgency) => {
    const data = await apiRequest('/auth/register', {
      method: 'POST',
      body: JSON.stringify({ companyName, email, password, isBgvAgency }),
    });
...
// Report API
const reportAPI = {
  // Note: PDF download is now handled directly in VerificationWizard component
  // to support direct file download instead of base64 URLs
};
