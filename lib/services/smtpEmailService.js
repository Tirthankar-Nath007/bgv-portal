...
import { getAllAdminEmails } from '@/lib/sql.data.service';
...
if (!SMTP_HOST || !FROM_EMAIL) {
  console.warn('[SMTP] Missing required environment variables: SMTP_HOST, SMTP_FROM_EMAIL');
}
...
async function getAdminRecipients() {
  try {
    ...
    console.error('[SMTP] Failed to fetch admin emails from DB:', err.message);
  }
  return ADMIN_EMAIL;
}
...
export async function sendAppealNotificationEmail(appeal, verifier, baseUrl) {
  ...
  if (!to) {
    console.warn('[SMTP] No admin recipients configured, skipping appeal notification');
...
export async function sendBlockNotificationEmail(data, baseUrl) {
  ...
  const to = await getAdminRecipients();
  if (!to) {
    console.warn('[SMTP] No admin recipients configured, skipping block notification');
    return { success: false, error: 'No admin recipients' };
  }
  return sendEmail(to, subject, html, text);
}
