...
export async function generateVerificationReportPDF(verificationData, employeeData) {
  try {
    ...
    // Return as data URL (can be used directly in browser)
    const filename = `verification-report-${verificationData.verificationId}.pdf`;
    
    console.log('[PDF] Generated PDF report:', filename, `(${Math.round(base64.length / 1024)} KB)`);

    return {
      s3Url: `data:application/pdf;base64,${base64}`, // Store as base64 data URL
      filename: filename,
      base64: base64 // Raw base64 for convenience
    };
...
export async function generateAppealResponsePDF(appealData) {
  try {
    ...
    return {
      s3Url: `data:application/pdf;base64,${base64}`,
