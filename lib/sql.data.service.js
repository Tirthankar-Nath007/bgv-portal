...
export async function findEmployeeByNumericId(numericId) {
    ...
    const sql = `SELECT id, employee_id, name, email, entity_name, date_of_joining, date_of_leaving, designation, exit_reason, fnf_status, department, created_at, updated_at FROM BGV_EMPLOYEES WHERE id = :id`;
    const rows = await executeQuery(sql, { id: idNum });
    if (rows.length > 0) {
        const emp = rows[0];
        return {
            id: emp.ID,
...
function formatAppeal(row) {
    const appealReasonVal = row.APPEAL_REASON;
    let appealReasonStr = '';
    if (appealReasonVal && typeof appealReasonVal === 'object') {
        if (appealReasonVal.getString) {
            appealReasonStr = appealReasonVal.getString();
        } else if (appealReasonVal.length > 0) {
            appealReasonStr = String(appealReasonVal);
        } else {
            appealReasonStr = '';
        }
    } else {
    ...
    return {
        ...
        appealReason: appealReasonStr,
        documents: safeJSONParse(row.DOCUMENTS) || [],
        mismatchedFields: safeJSONParse(row.MISMATCHED_FIELDS) || [],
        status: row.STATUS,
        hrResponse: String(row.HR_RESPONSE || ''),
        hrComments: String(row.HR_COMMENTS || ''),
