...
import { findEmployeeById as findEmpById, getEmployees as getEmps, findEmployeeRPTDBUAT } from '@/lib/services/employeeDataService';
...
export async function getVerifiersByIds(ids) {
    if (!ids || ids.length === 0) return {};
    await ensureDBConnection();
    const uniqueIds = [...new Set(ids.map(id => parseInt(id)))];
    const sql = `SELECT id, email, company_name, is_bgv_agency FROM BGV_VERIFIERS WHERE id IN (${uniqueIds.map((_, i) => `:id${i}`).join(',')})`;
    const params = {};
    uniqueIds.forEach((id, i) => { params[`id${i}`] = id; });
...
export async function getVerificationRecordsByIds(ids) {
    if (!ids || ids.length === 0) return {};
    await ensureDBConnection();
    const uniqueIds = [...new Set(ids.map(id => parseInt(id)))];
    const sql = `SELECT id, verification_id, employee_id, verifier_id, submitted_data, comparison_results, overall_status, match_score FROM BGV_VERIFICATION_RECORDS WHERE id IN (${uniqueIds.map((_, i) => `:id${i}`).join(',')})`;
    const params = {};
    uniqueIds.forEach((id, i) => { params[`id${i}`] = id; });
    const rows = await executeQuery(sql, params);
    const map = {};
    for (const v of rows) {
        map[v.ID] = {
            ...
            submittedData: safeJSONParse(v.SUBMITTED_DATA) || {},
            comparisonResults: safeJSONParse(v.COMPARISON_RESULTS) || [],
...
export async function addAdmin({ username, email, password, fullName, role, department, permissions }) {
    await ensureDBConnection();
    const sql = `INSERT INTO BGV_ADMINS (username, email, password, full_name, role, department, permissions, is_active)
                 VALUES (:username, :email, :password, :fullName, :role, :department, :permissions, 1)`;
...
export async function getAllAdminEmails() {
    await ensureDBConnection();
    const sql = `SELECT email FROM BGV_ADMINS WHERE is_active = 1 AND email != 'admin@bgvportal.com'`;
...
export async function findVerificationRecord(verificationId) {
    await ensureDBConnection();
    const sql = `SELECT * FROM BGV_VERIFICATION_RECORDS WHERE UPPER(verification_id) = UPPER(:verificationId)`;
    ...
    if (rows.length > 0) {
        ...
        return {
            ...
            overallStatus: v.OVERALL_STATUS,
            matchScore: v.MATCH_SCORE,
            consentGiven: v.CONSENT_GIVEN === 1,
            verificationCompletedAt: v.VERIFICATION_COMPLETED_AT,
...
export async function findVerificationRecordById(id) {
    await ensureDBConnection();
    const sql = `SELECT * FROM BGV_VERIFICATION_RECORDS WHERE id = :id`;
...
export function getBlockedMessage() {
    const adminEmail = process.env.ADMIN_EMAIL || 'the administrator';
    return `Maximum attempts reached. Please contact the exit team at ${adminEmail}`;
}

export async function getBlockedVerifiers() {
    await ensureDBConnection();

    const sql = `
        SELECT va.id, va.verifier_id, va.employee_id, va.attempt_count,
               va.is_blocked, va.blocked_at, va.last_attempt_at, va.created_at, va.updated_at,
               v.company_name, v.email AS verifier_email
        FROM BGV_VERIFICATION_ATTEMPTS va
        JOIN BGV_VERIFIERS v ON va.verifier_id = v.id
        WHERE va.is_blocked = 1 OR va.blocked_at IS NOT NULL
        ORDER BY va.blocked_at DESC NULLS LAST, va.last_attempt_at DESC
    `;
    ...
    const records = rows.map(row => ({
        ...
        verifierCompanyName: row.COMPANY_NAME,
        verifierEmail: row.VERIFIER_EMAIL,
        employeeId: row.EMPLOYEE_ID ? row.EMPLOYEE_ID.toString() : '',
    ...
    const uniqueEmpIds = [...new Set(records.map(r => r.employeeId).filter(Boolean))];
    const empNameMap = {};
    const t = Date.now();

    for (const empId of uniqueEmpIds) {
        try {
            const employee = await findEmployeeRPTDBUAT(empId);
            empNameMap[empId] = employee?.name || 'N/A';
        } catch (err) {
            console.error(`[BLOCKED_VERIFIERS] RPTDBUAT lookup failed for ${empId}:`, err.message);
            empNameMap[empId] = 'N/A';
        }
    }

    console.log(`[BLOCKED_VERIFIERS] Loaded ${Object.keys(empNameMap).length} employee names in ${Date.now() - t}ms`);

    return records.map(r => ({
        ...r,
        employeeName: empNameMap[r.employeeId] || 'N/A'
    }));
}
