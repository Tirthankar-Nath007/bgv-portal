...
async function apiRequest(endpoint, options = {}) {
    ...
    try {
        ...
        // Handle 403 - Role mismatch or corrupted session
        if (response.status === 403) {
            ...
            console.error('🚫 [apiRequest] 403 FORBIDDEN!');
            console.error('    - Error:', errorMessage);

            // If it's a role/access error, clear session and redirect
            if (errorMessage.includes('access required') || errorMessage.includes('role')) {
                console.error('    - Session appears corrupted or role mismatch');
                localStorage.removeItem('verifier_session');
                localStorage.removeItem('admin_session');
                localStorage.removeItem('auth_token');
            ...
            // Return the 403 error for non-session issues (like blocked verification)
            return { success: false, message: errorMessage, status: 403 };
        }
